...
async def answer_add_filters(message: Message, state: FSMContext):
    ...
    city, min_price, max_price = result if result else ("Не указано", "Не указано", "Не указано")
...
async def choose_city(message: Message, state: FSMContext):
    city = message.text
    if city != "Назад":
        cursor.execute("SELECT city_name_en FROM cities WHERE city_name_ru = ?", (city,))
        city_result = cursor.fetchone()

        if city_result:  # Если город найден
            city = city_result[0]  # ✅ Извлекаем строку из кортежа
            cursor.execute("UPDATE clients SET city = ? WHERE chat_id = ?", (city, chat_id))
            connection.commit()
